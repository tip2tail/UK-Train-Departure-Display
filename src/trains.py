...
def loadDeparturesForStationRtt(journeyConfig, username, password):
    if journeyConfig['departureStation'] == "":
        raise ValueError("Please set the departureStation value in config.json")

    if username == "" or password == "":
        raise ValueError("Please complete the transportApi_rttUsername and transportApi_rttPassword values in config.json")
    ...
    if ('destinationStation' in journeyConfig and
        ...
        apiUrl = f"https://api.rtt.io/api/v1/json/search/{departureStation}/to/{destinationStation}"
    else:
        apiUrl = f"https://api.rtt.io/api/v1/json/search/{departureStation}"

    r = requests.get(url=apiUrl, auth=HTTPBasicAuth(username, password))
    data = r.json()
...
def loadDeparturesForStation(journeyConfig, appId, apiKey):
    ...
    if appId == "" or apiKey == "":
        raise ValueError("Please complete the transportApi section of your config.json file")

    departureStation = journeyConfig['departureStation']

    apiUrl = f"http://transportapi.com/v3/uk/train/station/{departureStation}/live.json"
    apiParams = {
        'app_id':       appId,
        'app_key':      apiKey,
...
def loadDestinationsForDepartureRtt(journeyConfig, serviceId, serviceDate, username, password, showCompany):
    ...
    apiUrl = f"https://api.rtt.io/api/v1/json/service/{serviceId}/{serviceDate}"
    r = requests.get(url=apiUrl, auth=HTTPBasicAuth(username, password))
    data = r.json()
